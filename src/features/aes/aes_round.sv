...
for (genvar i = 0; i < 16; i = i + 1) begin: l_byte_split
...
for (genvar i = 0; i < 16; i = i + 1) begin: l_subbytes
    aes_sbox u_sbox (
        .byte_in (b[i])
      , .byte_out(sb[i])
    );
...
for (genvar i = 0; i < 16; i = i + 1) begin: l_pack_shift
    assign shifted_state[127 - i * 8 -: 8] = sh[i];
end
