...
module fetch_rvc_01_aligned_32_regression_tb;
  ...
  initial begin
    ...
    imem[0] = 32'h00A00093; // PC 0x00: addi x1,x0,10
    imem[1] = 32'h00100113; // PC 0x04: addi x2,x0,1
    imem[2] = NOP;

    release_reset();

    wait_fetch(32'h00000000, 32'h00A00093, "aligned 32-bit at PC 0x00");
    wait_fetch(32'h00000004, 32'h00100113, "aligned 32-bit at PC 0x04");
    wait_fetch(32'h00000008, NOP, "aligned NOP at PC 0x08");

    $finish;
  end

  `SETUP_VCD_DUMP(fetch_rvc_01_aligned_32_regression_tb)
