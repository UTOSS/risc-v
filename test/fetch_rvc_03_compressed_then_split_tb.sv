...
module fetch_rvc_03_compressed_then_split_tb;
  ...
  initial begin
    ...
    imem[0] = 32'h00930001; // PC 0x00: C.NOP, PC 0x02: low half of ADDI
    imem[1] = 32'h000100A0; // PC 0x04: high half of ADDI, PC 0x06: C.NOP
    imem[2] = NOP;

    release_reset();

    wait_fetch(32'h00000000, NOP, "C.NOP before split at PC 0x00");
    wait_fetch(32'h00000002, NOP, "bubble for split at PC 0x02");
    wait_fetch(32'h00000002, 32'h00A00093, "real split ADDI at PC 0x02");
    wait_fetch(32'h00000006, NOP, "C.NOP after split at PC 0x06");

    $finish;
  end

  `SETUP_VCD_DUMP(fetch_rvc_03_compressed_then_split_tb)
