...
module fetch_rvc_08_branch_to_aligned_32_tb;
  ...
  initial begin
    ...
    imem[0] = NOP;
    imem[1] = NOP;
    imem[2] = 32'h00A00093; // PC 0x08: addi x1,x0,10
    imem[3] = 32'h00100113; // PC 0x0C: addi x2,x0,1

    release_reset();

    redirect_to(32'h00000008);

    wait_fetch(32'h00000008, 32'h00A00093, "branch target aligned 32-bit at PC 0x08");
    wait_fetch(32'h0000000C, 32'h00100113, "fallthrough aligned 32-bit at PC 0x0C");

    $finish;
  end

  `SETUP_VCD_DUMP(fetch_rvc_08_branch_to_aligned_32_tb)
