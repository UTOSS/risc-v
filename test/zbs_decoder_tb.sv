...
`ifdef UTOSS_RISCV_ENABLE_B_EXT
    ...
    #1;
    assert (alu_control == ALU_CONTROL_BSET) else
      $fatal(1, "bset decode failed: got %0h", alu_control);

    instr = make_r_zbs(7'b0100100, 3'b001, 5'd3, 5'd2, 5'd1); // bclr
    #1;
    assert (alu_control == ALU_CONTROL_BCLR) else
      $fatal(1, "bclr decode failed: got %0h", alu_control);

    instr = make_r_zbs(7'b0110100, 3'b001, 5'd3, 5'd2, 5'd1); // binv
    #1;
    assert (alu_control == ALU_CONTROL_BINV) else
      $fatal(1, "binv decode failed: got %0h", alu_control);

    instr = make_r_zbs(7'b0100100, 3'b101, 5'd3, 5'd2, 5'd1); // bext
    #1;
    assert (alu_control == ALU_CONTROL_BEXT) else
      $fatal(1, "bext decode failed: got %0h", alu_control);

    instr = make_i_zbs(7'b0010100, 5'd3, 5'd2, 5'd1, 3'b001); // bseti
    #1;
    assert (alu_control == ALU_CONTROL_BSET) else
      $fatal(1, "bseti decode failed: got %0h", alu_control);

    instr = make_i_zbs(7'b0100100, 5'd3, 5'd2, 5'd1, 3'b001); // bclri
    #1;
    assert (alu_control == ALU_CONTROL_BCLR) else
      $fatal(1, "bclri decode failed: got %0h", alu_control);

    instr = make_i_zbs(7'b0110100, 5'd3, 5'd2, 5'd1, 3'b001); // binvi
    #1;
    assert (alu_control == ALU_CONTROL_BINV) else
      $fatal(1, "binvi decode failed: got %0h", alu_control);

    instr = make_i_zbs(7'b0100100, 5'd3, 5'd2, 5'd1, 3'b101); // bexti
    #1;
    assert (alu_control == ALU_CONTROL_BEXT) else
      $fatal(1, "bexti decode failed: got %0h", alu_control);
`else
